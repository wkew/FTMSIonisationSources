...
class UpSetPlot():
    ...
    def _base_sets_plot(self, sorted_sets, sorted_set_names):
        ...
        ax.invert_xaxis()
        height = .6
        bar_bottoms = self.y_values# - height / 6
        ...
        ax.set_xlabel("Set size", fontweight='bold', fontsize=14)
    ...
    def _inters_sizes_plot(self, ordered_in_sets, inters_sizes):
        ...
        for x, y in zip(self.x_values, inters_sizes):
            ax.text(x, y + label_vertical_gap, "%.0f" % y,
                    rotation=90, ha='center', va='bottom',size=14)
        ...
        ax.set_ylabel("Intersection size", labelpad=6, fontweight='bold', fontsize=14)
        ax.tick_params(axis='both',labelsize=14)
        return ax.get_xlim()
    ...
    def additional_plot(self, ax_index, kind, data_values, graph_args, *, labels=None):
        ...
        for l, text in labels.items():
            getattr(ax, 'set_%slabel' % l)(text, labelpad=3,
                                           fontweight='bold', fontsize=14) if l in ['x', 'y'] else None
        return ax
