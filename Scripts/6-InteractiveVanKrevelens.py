# -*- coding: utf-8 -*-
"""
Created on Sun Feb 18 15:17:58 2018
...
def intplotter(data,isodata,nodata,y):#,hetclassintdf):
    ...
    p1 = figure(tools=TOOLS, title=y[:-5]+" - Van Krevelen",width=figdims[0], height=figdims[1], x_axis_label='O/C',y_axis_label='H/C',x_range=vkxlim,y_range=vkylim)
    ...
    p2 = figure(tools=TOOLS, title=y[:-5]+" - DBE vs C# Plot",width=figdims[0], height=figdims[1], x_axis_label='C#',y_axis_label='DBE',x_range=dbexlim,y_range=dbeylim)
    color_mapper2 = LinearColorMapper(palette=glocmap2, low=0, high=cmax)
    p2.scatter(x='C', y='DBE',source=source,size='VKsize', fill_color={'field': 'O', 'transform': color_mapper2}, fill_alpha=0.75,line_color=None)
    ...
    p4 = figure(tools=TOOLS, title=y[:-5]+" - Assigned Centroid MS",width=figdims[0], height=figdims[1], x_axis_label='m/z',y_axis_label='Abundance',y_range=[min(data["RA"]),max(data["RA"])],x_range=msxlim)
    ...
    p5 = figure(tools=TOOLS, title=y[:-5]+" - Assigned Centroid MS",width=1400, height=600, x_axis_label='m/z',y_axis_label='Abundance', y_range=[min(data["RA"]),max(data["RA"])],x_range=(msxlim[0]-stretch,msxlim[1]+stretch))
